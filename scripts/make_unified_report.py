...
def main() -> None:
    ...
    for r in rows:
        lines.append(
            f"| {r['K']} | {r['algo']} | {r['final']:.4f} | {r['best']:.4f} | "
            f"{r['r90'] if r['r90'] else '-'} | {'yes' if r['plateau'] else 'NO'} | "
            f"{r['mad_last5']*100:.2f}pp |"
        )
    lines += [
        "",
        ...
        f"change over the last 5 rounds is < {PLATEAU_TOL*100:.1f}pp. Only",
